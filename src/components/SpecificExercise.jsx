	...
	const getExercises = async () => {
		try {
			const response = await fetch(
				`https://exercisedb.p.rapidapi.com/exercises/bodyPart/${title.toLowerCase()}?limit=30&offset=0`,
				{
					method: "GET",
					headers: {
	...
	return (
		<SafeAreaView style={tw`flex-1 bg-white`}>
			<ScrollView contentContainerStyle={tw`p-4`} showsVerticalScrollIndicator={false} bounces={true}>
				...
				<Image
					source={{ uri: image }}
					style={[tw`self-center mt-6 rounded-full border-2 border-pink-600`, { width: 220, height: 220 }]}
					resizeMode="cover"
				/>

				<View style={tw`mt-8`}>
					<Text
						style={tw`text-lg font-semibold mb-4 text-center border-solid border-1 bg-gray-50 w-1/2 self-center p-2 rounded-lg`}>
						Available Workouts
					</Text>
					{loading ? (
                    ...
                    navigation.navigate("ExerciseDetails", { exercise: item });
									...
									/>
									<Text style={tw`text-black text-1xl font-semibold text-center`}>
